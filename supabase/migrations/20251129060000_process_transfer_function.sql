-- Migration: create process_transfer RPC function
-- Adds an atomic transfer function that debits the from account, credits an internal to account (if present),
-- inserts a transactions row, and returns (success boolean, error text).
-- NOTE: This function uses SECURITY DEFINER to allow Edge Functions (or service-role callers) to run it while keeping RLS in place.
-- Review SECURITY/OWNER and RLS settings to match your Supabase setup.
